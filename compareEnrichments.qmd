...
entrezUniverseSmall <- unique(c(sample(entrezUniverse, size=15000), entrezList1))
```
...
```{r}
GOparamsSmall = new("GOHyperGParams",
    geneIds=entrezList1, 
    universeGeneIds=entrezUniverseSmall,
    annotation="org.Hs.eg.db", 
    ontology="BP",
    pvalueCutoff=0.1, # Hyper geometric test unadjusted p-value
    minSizeCutoff =1
...
GOhyperSmall = hyperGTest(GOparamsSmall)
resGOstatSmall <- summary(GOhyperSmall)
```
...
## Una altre comparacio

```{r}
tableAll1 <- matrix (c(3,21, 48-3, 18800-21), nrow=2, byrow=TRUE)
fisher.test(tableAll1, alternative="greater")
tableSkato1 <- matrix (c(3,14, 48-3, 8865-14), nrow=2, byrow=TRUE)
fisher.test(tableSkato1, alternative="greater")
```
